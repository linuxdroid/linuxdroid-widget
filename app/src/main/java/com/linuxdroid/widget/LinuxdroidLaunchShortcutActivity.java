 ...
 */
public class LinuxdroidLaunchShortcutActivity extends Activity {

	static final String TOKEN_NAME = "com.linuxdroid.shortcut.token";
	...
	protected void onResume() {
		...
		if (token == null || !token.equals(getGeneratedToken(this))) {
			Log.w("linuxdroid", "Strange token: " + token);
		...
		LinuxdroidWidgetProvider.ensureFileReadableAndExecutable(clickedFile);
		...
		Intent executeIntent = new Intent(LinuxdroidWidgetProvider.ACTION_EXECUTE, scriptUri);
		executeIntent.setClassName("com.linuxdroid", LinuxdroidWidgetProvider.LINUXDROID_SERVICE);
		...
		LinuxdroidWidgetProvider.startLinuxdroidService(this, executeIntent);
		finish();
	}
}
